...
- `docshell`, `toolkit/content` и панель доверия Fenix — внутренняя страница
  ...
  «Продолжить один раз» и «Разрешить этот домен». Сведения о сертификате
  и параметры исключения раскрываются прямо в предупреждении. Из «щита»
  открываются домены текущей страницы с галочками для заблокированных адресов;
  общие настройки и сохранённые исключения вынесены на отдельный экран;
...
### Выпуски при изменении SCT и корневого сертификата

Ежедневная проверка учитывает стабильную версию Firefox и данные защиты в
репозитории: `native/ct-log-list.json` и сертификат
`certificates/russian_trusted_root_ca.pem`. Сертификат проверяется по закреплённому
отпечатку в `certificates/ministry-ca-lock.json`. Автоматической загрузки новых
доверенных сертификатов или списков журналов из внешних источников нет.

Если версия Firefox не изменилась, но данные защиты обновлены, создаётся отдельный
релиз: например, `155.0.1_SCTch`, затем `155.0.1_SCTch2`, `155.0.1_SCTch3`.
Он отмечается как Latest для программ отслеживания GitHub-релизов, включая Obtainium.
Новая версия Firefox получает обычный тег, например `156.0`.

Изменения кода, README и форматирования JSON/PEM сами по себе не вызывают такую
пересборку. Отпечаток данных сохраняется в `source.json` и сравнивается с последним
выпуском той же версии Firefox. Для старых выпусков без отпечатка используются
данные исходного коммита из `source.json`; если самих метаданных нет, выполняется
одна пересборка. Ошибка GitHub API останавливает проверку.

Суффикс добавляется к тегу, названию релиза и APK-файлам. Версия исходного Firefox
остаётся прежней; Android `versionCode` растёт с номером запуска workflow.
Ручной запуск по-прежнему пересобирает текущий выпуск; новый `_SCTch` создаётся
только при изменении данных защиты, а не при каждом ручном запуске.
